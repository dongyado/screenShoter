...
import com.dongyado.config.SShoterConfig;
...
public class Shoter extends JPanel{
	...
	public JPanel getButtons(){
		...
		buttonSave.registerKeyboardAction(buttonSaveLis, KeyStroke.getKeyStroke(KeyEvent.VK_S, KeyEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW); 
		...
		buttonCancel.registerKeyboardAction(cancelLis, KeyStroke.getKeyStroke(KeyEvent.VK_D, KeyEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW); 
		...
		buttonExit.registerKeyboardAction(buttonExitLis, KeyStroke.getKeyStroke(KeyEvent.VK_Q, KeyEvent.CTRL_DOWN_MASK), JComponent.WHEN_IN_FOCUSED_WINDOW); 
	...
	public void paintComponent(Graphics comp) {
		...
		BasicStroke stroke = new BasicStroke(1f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER);
		comp2D.setStroke(stroke);
		
		Rectangle _rect = new Rectangle(_start, new Dimension(_end.x - _start.x, _end.y - _start.y));
		comp2D.draw(_rect);
		Color ColorFill = new Color(255, 255, 255, 0);
		comp2D.setColor(ColorFill);
		comp2D.fill(_rect);
	...
	private class RectDragListener implements MouseMotionListener {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if( operationStatus == SShoterConfig.MOUSE_DRAG ){
				...
				if( e.getX() - _moveStart.getX() >= 0 &&  e.getX() - _moveStart.x + rectSize.width <= SShoterConfig.SCREEN_SIZE.width){
				...
				if( e.getY() - _moveStart.getY() >= 0 &&  e.getY() - _moveStart.y + rectSize.height <= SShoterConfig.SCREEN_SIZE.height){
		...
		//	sshoter.getWindow().setAutoRequestFocus(true);
			...
			if( operationStatus == SShoterConfig.MOUSE_DRAG ) return;
			//in the sub image 
			if( e.getX() > _start.getX() + SShoterConfig.MOVE_EDGE 
			...
			//out the sub image
			}else if( e.getX() < _start.getX() || e.getX() > _end.getX() || e.getY() < _start.getY() || e.getY() > _end.getY()){	
			...
			//in the sub image edge area
				...
				if( e.getX() >= _start.getX() && e.getX() <= _start.getX() + SShoterConfig.MOVE_EDGE ){
					...
					}else if( e.getY() >= _end.getY() - SShoterConfig.MOVE_EDGE &&
							e.getY() <= _end.getY()){
	...
	public BufferedImage getNewImage( ){
		...
		return this.screenImg.getSubimage( (int)_start.getX(), (int)_start.getY(), (int)( _end.getX() - _start.getX()) , (int)( _end.getY() - _start.getY() ));
